...
public class CustomerController {
    ...
    public ResponseEntity<Response<CustomerDTO>> createCustomer(@RequestHeader(value = CustomersApiUtil.HEADER_CUSTOMERS_API_VERSION, defaultValue = "${api.version}") @Parameter(description = " api version for this endpoint ") String apiVersion,
        ...
        CustomerDTO customerToCreate = customerService.save(customer);


        response.setData(customerToCreate);
        ...
        log.info("Create Customer Service Executed successfully",customerToCreate.getCustomerId());
    ...
    public ResponseEntity<Response<CustomerDTO>> updateCustomer(@RequestHeader(value = CustomersApiUtil.HEADER_CUSTOMERS_API_VERSION, defaultValue = "${api.version}") String apiVersion,
        ...
        CustomerDTO customerToFind = customerService.findById(dto.getCustomerId())
        ...
        Customer customer = dto.convertDTOToEntity();
        CustomerDTO customerToUpdate = customerService.save(customer);

        response.setData(customerToUpdate);
 ...
 /*   @GetMapping(value = "/byOrderNumber/{orderNumber}")
    ...
    @GetMapping("/list")
    @Operation(summary  = "Route to find all customers of the API in a period of time")
    public ResponseEntity<CustomersDTO> findAllCustomers(@RequestHeader(value = CustomersApiUtil.HEADER_CUSTOMERS_API_VERSION, defaultValue = "${api.version}")
                                                                           String apiVersion, @RequestHeader(value = CustomersApiUtil.HEADER_API_KEY, defaultValue = "${api.key}") String apiKey,
                                                         @PageableDefault(page = 1, size = 10, sort = {"customerId"}) Pageable pageable) throws CustomerNotFoundException {
        ...
        return new ResponseEntity<>(new CustomersDTO(customers), headers, HttpStatus.OK);
    }
